...
import { Send, FileText, Calendar, Plane, CreditCard, Clock, User, ShieldCheck } from "lucide-react";
...
export default function ClientApplicationDetail() {
  ...
  if (isLoading) return <div className="p-12 text-center text-muted-foreground">Chargement des détails...</div>;
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-border shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Ref: JOV-{app.id.toString().padStart(5, '0')}</p>
            </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-border shadow-sm">
          ...
          <div className="relative border-l-2 border-slate-100 ml-4 space-y-8 pb-4">
            {timelineSteps.map((step, idx) => {
              ...
              const isError = app.status === 'rejected' && step.key === 'approved'; // replace last step with error visually
              ...
              return (
                <div key={step.key} className="relative pl-8">
                  <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 bg-white ${
                    ...
                    isCurrent ? "border-secondary bg-yellow-100" :
